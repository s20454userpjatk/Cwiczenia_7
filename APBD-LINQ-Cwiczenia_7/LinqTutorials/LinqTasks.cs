...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            var result = Emps.Where(x => x.Job == "Backend programmer");
            return result;
        }
        ...
        {
            var result = Emps.Where(x => x.Salary == Emps.Max(x => x.Salary));
            return result;
        }
        ...
        {
            var result = Emps.GroupBy(x => x.Job).Take(1).Select(x => new
            {
                Praca = x.Key,
        ...
        {
            Emp result = Emps.OrderByDescending(x => x.HireDate)
                .FirstOrDefault();
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Select(x => new
            {
                x.Ename,
                x.Job,
                x.HireDate
            ...
            var collection = new[]
            {
                new
                {
                    Ename = "Brak wartości",
                    Job = (object)null,
                    Hiredate = (object)null
                }
            };

            return result.Union(collection);
        }
        ...
        {
            IEnumerable<object> result = Depts.GroupJoin(Emps, d => d.Deptno, e => e.Deptno, (d, e) => new
            {
                Name = d.Dname,
        ...
        public static IEnumerable<Dept> Task14()
        {
            IEnumerable<Dept> result = Depts.GroupJoin(Emps, d => d.Deptno, e => e.Deptno, (d, e) => new
            {
                Dept = d,
                NumOfEmployees = e.Count()
            }).Where(x => x.NumOfEmployees == 5 || x.NumOfEmployees == 0)
            .Select(x => x.Dept);
        ...
        {
            IEnumerable<object> result = Emps.Where(x => x.Job.Contains('A', StringComparison.InvariantCultureIgnoreCase))
                .GroupBy(e => e.Job)
                .Where(g => g.Count() > 2)
                .Select(x => new
                {
        ...
        {
            IEnumerable<object> result = Emps.SelectMany(e => Depts, (e, d) =>
            new
            {
                e.Empno,
                e.Ename,
